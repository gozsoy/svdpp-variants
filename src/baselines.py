...
train_valid_df, test_df = train_test_split(
    ratings_df, test_size=test_size, random_state=test_split_random_state)
train_df, valid_df = train_test_split(
    train_valid_df, test_size=valid_size,
    random_state=valid_split_random_state)
...
y_pred_dummy = np.ones((len(test_df.rating.values))) * \
    np.mean(train_valid_df.rating)
y_true = test_df.rating.values
print(f'baseline 2 test rmse: {rmse(y_true,y_pred_dummy)}')

# Baseline 3: predict user mean rating
user_means = train_valid_df.groupby('user_id').agg(
...
item_means = train_valid_df.groupby('movie_id').agg(
    {'rating': 'mean'}).reset_index()
